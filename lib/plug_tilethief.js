...
function TileThief() {
	...
	this.queue = async.queue(function (task, callback) {
		caller.request(task.tile_url, task.tile_file, function (err, data) {
			if (err)
				task.cb(err)
			else
				task.cb(null, data);
...
TileThief.prototype.send = function (mapname, x, y, z, format, cb) {
	var map = config.tilethief.maps[mapname];
	...
	fs.exists(tile_file, function (e) {
		if (e) {
			...
			mkdirp(path.dirname(tile_file), function (err) {
				...
				caller.queue.push({tile_url: tile_url, tile_file: tile_file, cb: cb}, function (err) {
					if (err) {
