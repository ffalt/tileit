		...
		create: function (cb) {
			try {
				...
				if (options.xml) {
					...
					options.initMap(map, mapnik, mapnikDefs.MapnikDef, options, function (err) {
						error = err;
						cb(err, map);
					});
				} else {
...
Mapnik.prototype.validateMapOptions = function (mapname, options) {
	if (options.buildDef)
		options.buildDef(mapnikDefs.MapnikDef, options);
};
...
Mapnik.prototype.requestRenderMetaImage = function (meta_req, options) {
	...
	aquire(stylesheet, options, function (err, map) {
		...
		if (err || (!map)) {
			caller.logger.error('[Mapnik] ' + err, options.xml);
			meta_req.error(err);
			cleanup();
		} else {
