...
var vectorFormats = ['pbf', 'json', 'utf'];
...
Mapnik.prototype.storeMetaTile = function (map, image, meta, options) {
	...
	async.forEachSeries(meta.getTiles(), function (tile, nextcb) {
		...
		meta.saveFile(path.resolve(options.metatilepath), function (tile) {
			return tile.buffer;
		}, function (err) {
			//cb();
		})
...
Mapnik.prototype.renderMetaImage = function (map, meta_req, options, cb) {
	...
	console.log('renderMetaImage', options.metatile, 'w:', meta.width, 'h:', meta.height);
...
Mapnik.prototype.requestMetaImage = function (treq, options) {
	...
	meta_req.readImage(options.metatilepath, function (err) {
		if (err) {
