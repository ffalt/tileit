...
MetaTile.prototype.saveFile = function (rootdir, getbuffer, cb) {
	...
	mkdirp(path.dirname(imgfile), function (err) {
		...
		caller.tiles.sort(function (a, b) {
			return caller.calcPos(a.x, a.y) - caller.calcPos(b.x, b.y);
		});

		caller.tiles.forEach(function (tile, j) {
			var buffer = getbuffer(tile);
			proxy.offsets[j].offset = offset;
			proxy.offsets[j].size = buffer.length;
			offset += buffer.length;
			list.push(buffer);
		});

		try {
			list.unshift(Header.buffer());
			fs.writeFile(imgfile + '_', Buffer.concat(list), function (err) {
				if (err) {
					...
					fs.rename(imgfile + '_', imgfile, cb);
				}
			});
		} catch (e) {
			cb(err);
