...
function MetaTile(x, y, z, options) {
	this.metatile = options.metatile || 8;
	...
	for (var dx = 0; dx < this.metaWidth; dx++) {
		for (var dy = 0; dy < this.metaHeight; dy++) {
			...
			if (this.tiles.length != this.calcPos(tile.x, tile.y)) {
				console.log('nope', this.tiles.length, this.calcPos(tile.x, tile.y), tile);
			}
	...
	fs.open(imgfile, 'r', null, function (err, fd) {
		if (err) {
			...
			fs.read(fd, buf, 0, length, 0, function (err, bytesRead) {
				...
				if (fields.magic != 'META') {
					cb('Metatile Header error :.(');
					fs.close(fd);
				} else if (err || bytesRead !== length) {
					cb('Metatile error :.(');
					fs.close(fd);
				} else {
					cb(null, {
	...
	mkdirp(path.dirname(imgfile), function (err) {
		if (err) {
			...
			try {
				...
				stream.write(Header.buffer());
				caller.tiles.forEach(function (tile) {
					stream.write(getbuffer(tile));
				});
				stream.end();
			} catch (e) {
				cb(err);
