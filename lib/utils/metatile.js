...
MetaTile.prototype.saveFile = function (rootdir, getbuffer, cb) {
	...
	mkdirp(path.dirname(imgfile), function (err) {
		if (err) {
			...
			try {
				/*
				 note if filewriting is async another request may try to read the
				 current written file and fail, so use sync instead
				...
				fs.writeFileSync(imgfile, Buffer.concat(list));
				cb();
			} catch (e) {
				cb(err);
			}
