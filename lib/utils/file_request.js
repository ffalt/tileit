...
function FileRequest(concurrent_requests, logger) {
	this.logger = logger;
	this.queue = async.queue(function (task, callback) {
		logger.debug('[FileRequest] Requesting:', task.url);
		request({url: task.url, encoding: null}, function (error, response, body) {
			...
			if (error) {
				logger.error("[FileRequest] Could not fetch tile from server", task.url, error);
				task.cb(error);
			} else {
				task.cb(null, body);
			}
...
//		}
	...
	this.queue.push({url: url, test_content_type: test_content_type, test: test, cb: cb}, function (err) {
		if (err) {
			caller.logger.error("[FileRequest] Error fetching tile:", url, err);
		}
	});
};
