...
Machine.prototype.init = function (plugs, config, cb) {
	...
	var files = fs.readdirSync(path.resolve(config.mapconfigpath));
	for (var i = 0; i < files.length; i++) {
		if (path.extname(files[i]).toLocaleLowerCase() == '.json') {
			var obj = JSON.parse(fs.readFileSync(path.resolve(config.mapconfigpath + '/' + files[i])).toString());
