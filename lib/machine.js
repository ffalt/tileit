...
Map.prototype.storeResultCB = function (storemapplugs, buffer, treq, cb) {
	var caller = this;
	async.forEach(storemapplugs, function (mapplug, next) {
		mapplug.plug.storeImage(treq, buffer, mapplug.options, function (err) {
			if (!err)
				global.logger.debug('[Tileit] Tile stored, map:', caller.name, ', plug:', mapplug.plug.name);
			next();
	...
	var check = function (i) {
		...
		treq.finish = function (err, buffer) {
			if ((err) || (!buffer)) {
				...
				if (!treq.waitForStorage) {
					finish(err, buffer);
					caller.storeResult(storemapplugs, buffer, treq);
				} else {
					caller.storeResultCB(storemapplugs, buffer, treq, function () {
						finish(err, buffer);
					});
				}
			}
...
Machine.prototype.warmCache = function (options, cb) {
	var me = this;
	options.maps = options.maps.map(function (mapname) {
	...
	collector.collect(options.maps, options.zooms, options.bbox, function (reqs) {
		var hasErrors = false;
		async.forEachSeries(reqs, function (req, callback) {
			req.lhc = me;
			req.waitForStorage = true;
			req.finish = function (err, data) {
				hasErrors = hasErrors || err || (!data);
				if (options.progress)
					options.progress(reqs.indexOf(req) + 1, reqs.length, req.map.name, req.z, req.x, req.y, req.map.format, err, callback);
				else
					callback();
			};
