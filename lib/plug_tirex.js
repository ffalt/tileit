...
try {
	...
	console.error('[Tirex] Are you running this on unix?');
}
...
function MetaRequest(map, meta_x, meta_y, z, logger) {
	this.requests = {};
	this.map = map;
	this.meta_x = meta_x;
	this.meta_y = meta_y;
...
MetaRequest.prototype.push_request = function (treq) {
	var fp = [treq.x, treq.y].join('/');
	this.requests[fp] = this.requests[fp] || [];
	this.requests[fp].push(treq);
};
...
MetaRequest.prototype.send_image = function (fd, req_array, cb) {
	var req = req_array[0];
	this.read_image(req.x, req.y, fd, function (err, buffer) {
...
// create a queue object with concurrency 1
	...
	for (var key in this.requests) {
		q.push({name: key}, function (err) {
			if (err) {
				caller.logger.error('[Tirex] Error on sending:', key);
			} else {
				caller.logger.debug('[Tirex] Finished sending:', key);
			}
		});
	}
};
...
MetaRequest.prototype.timeout = function () {
	...
	q.drain = function () {
		caller.logger.debug('[Tirex] Timeout cancel done');
	};

	for (var key in this.requests) {
		q.push({name: key}, function (err) {
			if (err)
				caller.logger.error('[Tirex] Error calling timeout:', key, ':', err);
		});
	}
};
...
function Tirex(name, settings, logger) {
	...
	try {
		...
		caller.logger.error('[Tirex] Could not connect to Tirex Master', e);
	}
...
Tirex.prototype.getImage = function (treq, options) {
	...
	if (this.requests[l]) {
		...
		this.requests[l].push_request(treq);
		return;
	}
	var caller = this;
	var meta_req = new MetaRequest(map, meta_x, meta_y, treq.z, this.logger);
	meta_req.push_request(treq);
...
Tirex.prototype.request_tirex = function (meta_req) {
	...
	caller.logger.debug("[Tirex] message to tirex:", JSON.stringify(msg));
