...
var master_socket = '/run/tirex/master.sock';
...
function Tirex() {
	...
	this.sock = unix.createSocket('unix_dgram', function (message, rinfo) {
		...
		console.debug('[Tirex] Message: ' + s + ' from ' + rinfo.address + ':' + rinfo.port);
...
Tirex.prototype.request_tirex = function (map, x, y, z, cb) {
	...
	console.debug("[Tirex] message to tirex\n", JSON.stringify(msg));
	var buf = new Buffer(s);
	this.sock.send(buf, 0, buf.length, master_socket, function (err) {
		if (err) {
